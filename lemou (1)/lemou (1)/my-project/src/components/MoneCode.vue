<template>
  <div>
    <header>
      ...
      <span>{{amount}}￥支付宝固码</span>
    </header>
    <div class="Co">
      <van-button type="primary" size="large">点击上传支付宝固码</van-button>
      <van-uploader
      ...
      <div class="Clads">
        <div class="geren" v-for="(item,index) in datas" :key="index">
          ...
          <div class="rioimg">
            <p>
              {{item.typeCode}}
              <span style="color:red">({{(item.amount)/100}}元)</span>
            </p>
...
export default {
  ...
  created() {
    this.amount = this.$route.query.MoenyC;
  },
  mounted() {
    this.init();
  },
