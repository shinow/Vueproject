<template>
  <div>
    ...
    <div class="Co">
      ...
      <van-uploader :after-read="afterRead" preview-size="3.7rem" style="overflow:hidden;height:0.5rem;border:1px solid red"/>
    </div>
    ...
    <div class="geren" v-for="(item,index) in datas" :key="index">
      ...
      <div class="rioimg">
        ...
        <p style="font-size:0.16rem;">{{item.typeCode}}状态：
<span :style="{color:item.color}">{{item.status}}</span></p>
        <div class="Chimg">
  ...
  methods: {
    ...
    Sconfirm() {
      ...
      if (this.Updacode == "上传二维码") {
        ...
        if (this.Sparams===""||this.Sparams==null||this.Sparams==undefined) {
              var pay = {
                code: this.Wcode,
                id: this.Imgid,
                name: this.nameA
        ...
        this.axios
          ...
          .then(res => {
            ...
            if (res.httpCode == 0) {
              this.Http.post("/api/cjtpay-commercial-web/account/update", pay).then(data => {
                this.$toast("修改成功！");
                  this.Updacode = "上传二维码"
                 this.Di=false
                this.init();
