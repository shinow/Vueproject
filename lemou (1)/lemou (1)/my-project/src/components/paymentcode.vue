<template>
  <div>
    ...
    <div class="Co">
      <van-button type="primary" size="large">点击上传支付宝收款码</van-button>
      <van-uploader
    ...
    <div class="geren" v-for="(item,index) in datas" :key="index">
      ...
      <div class="rioimg">
        ...
        <p style="font-size:0.16rem;">{{item.typeCode}}状态：<span :style="{color:item.color}">{{item.status}}</span></p>
        <div class="Chimg">
    ...
    <van-dialog v-model="show" @cancel="cancelD" @confirm="Sconfirm" show-cancel-button>
      <div style="text-align: center" class="imll">
        <p style="font-size:0.2rem;margin-top:0.1rem;display:none">{{Updacode}}</p>
        <p style="font-size:0.2rem;margin-top:0.1rem" v-if="Di">{{Wcode}}</p>
        <img style="width:2rem;height:2rem;margin:0.1rem 0" :src="Shimg" />
        <van-uploader preview-size="1.7rem" :after-read="afterRead" />
        <div style="margin-top:0.1rem" v-if="Tyimg">
          <span>请选择类型：</span>
          <van-radio-group @change="changeRo" v-model="Wcode">
            <van-radio style="float: left;margin:0.1rem 0 0 0.3rem;" name="0">支付宝个码</van-radio>
            <van-radio style="float: left;margin:0.1rem 0 0 0.2rem" name="3">支付宝商码</van-radio>
          </van-radio-group>
        </div>
        <input
          type="text"
          style="width:3rem;height:0.5rem;margin:0.1rem 0; border:1px solid black; border-radius:0.2rem;text-align: center"
          v-model="nameA"
